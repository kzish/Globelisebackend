...
pub fn create_access_token(ulid: Ulid, email: EmailAddress) -> Result<String, Error> {
...
pub fn create_refresh_token(ulid: Ulid) -> Result<(String, i64), Error> {
    ...
    async fn decode(input: &str, database: Arc<Mutex<Database>>) -> Result<Self, Error> {
        ...
        if database.admin(ulid).await?.is_none() {
            return Err(Error::Unauthorized(
                "Access token rejected: admin does not exist",
            ));
        }
    ...
    B: Send,
{
    ...
    async fn from_request(req: &mut RequestParts<B>) -> Result<Self, Self::Rejection> {
        ...
        if database.admin(ulid).await?.is_none() {
            return Err(Error::Unauthorized(
                "Refresh token rejected: admin does not exist",
            ));
        }
